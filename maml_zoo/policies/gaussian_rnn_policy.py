...
class GaussianRNNPolicy(Policy):
    ...
    def build_graph(self):
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            with tf.compat.v1.variable_scope("log_std_network"):
                log_std_var = tf.compat.v1.get_variable(name='log_std_var',
                                              shape=(1, self.action_dim,),
                                              dtype=tf.float32,
                                              initializer=tf.compat.v1.constant_initializer(self.init_log_std),
    ...
    def reset(self, dones=None):
        ...
        if self._hidden_state is None:
            ...
            if isinstance(self._hidden_state, tf.nn.rnn_cell.LSTMStateTuple):
    ...
    def get_zero_state(self, batch_size):
        ...
        if isinstance(self._hidden_state, tf.nn.rnn_cell.LSTMStateTuple):
            ...
            hidden = tf.nn.rnn_cell.LSTMStateTuple(hidden_c, hidden_h)
            return hidden
        else:
