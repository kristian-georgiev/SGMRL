...
class DICEMAML(MAMLAlgo):
    ...
    Args:
        ...
        second_order_baseline (boolean): whether to use a second order baseline (Foerster et al.)
    ...
    def __init__(
        ...
        self._optimization_keys = ['observations', 'actions', 'rewards', 'baselines', 'mask', 'agent_infos']
        self.name = name
        self.second_order_baseline = second_order_baseline
    ...
    def _adapt_objective_sym(self, action_stacked_sym, reward_sym, baseline_sym, mask_sym, dist_info_stacked_sym):
        with tf.compat.v1.variable_scope("log_likelihood"):
            log_likelihood_adapt = self.policy.distribution.log_likelihood_sym(action_stacked_sym, dist_info_stacked_sym)
            log_likelihood_adapt = tf.reshape(log_likelihood_adapt, tf.shape(input=mask_sym))
        with tf.compat.v1.variable_scope("dice_loss"):
            dice_loss = - tf.reduce_mean(input_tensor=magic_box_cumsum(log_likelihood_adapt) * reward_sym * mask_sym)
            baseline_loss = - (1 - tf.reduce_mean(input_tensor=magic_box(log_likelihood_adapt) * baseline_sym * mask_sym))
            if self.second_order_baseline:
                second_order_bl_loss = (1 - magic_box(log_likelihood_adapt))[:, 1:] * \
                                  (1 - magic_box_cumsum(log_likelihood_adapt))[:, :-1] * baseline_sym[:, 1:] * mask_sym[:, 1:]
                second_order_bl_loss = tf.reduce_mean(input_tensor=second_order_bl_loss)
            else:
                second_order_bl_loss = 0
    ...
    def _build_inner_adaption(self):
        ...
        """
        obs_phs, action_phs, reward_phs, baseline_phs, mask_phs, dist_info_old_phs, adapt_input_ph_dict = self._make_dice_input_placeholders('adapt')

        adapted_policies_params = []

        for i in range(self.meta_batch_size):
            with tf.compat.v1.variable_scope("adapt_task_%i" % i):
                with tf.compat.v1.variable_scope("adapt_objective"):
                    ...
                    adapt_loss = self._adapt_objective_sym(action_stacked, reward_phs[i], baseline_phs[i], mask_phs[i],
                                                           distribution_info_stacked)

                # get tf operation for adapted (post-update) policy
                with tf.compat.v1.variable_scope("adapt_step"):
    ...
    def build_graph(self):
        ...
        with tf.compat.v1.variable_scope(self.name + '_sampling'):
        ...
        meta_update_scope = tf.compat.v1.variable_scope(self.name + '_meta_update')

        with meta_update_scope:
            obs_phs, action_phs, reward_phs, baseline_phs, mask_phs, dist_info_old_phs, all_phs_dict = self._make_dice_input_placeholders('step0')
        ...
        with meta_update_scope:
            ...
            for step_id in range(1, self.num_inner_grad_steps+1):
                with tf.compat.v1.variable_scope("inner_update_%i"%step_id):
                    surr_objs, adapted_policy_params = [], []

                    # inner adaptation step for each task
                    for i in range(self.meta_batch_size):
                        action_stacked = self._reshape_action_phs(action_phs[i])
                        surr_loss = self._adapt_objective_sym(action_stacked, reward_phs[i], baseline_phs[i], mask_phs[i], distribution_info_vars[i])
                ...
                obs_phs, action_phs, reward_phs, baseline_phs, mask_phs, dist_info_old_phs, all_phs_dict = self._make_dice_input_placeholders('step%i' % step_id)
            ...
            with tf.compat.v1.variable_scope("outer_update"):
                surr_objs =[]

                # meta-objective
                for i in range(self.meta_batch_size):
                    action_stacked = self._reshape_action_phs(action_phs[i])
                    surr_obj = self._adapt_objective_sym(action_stacked, reward_phs[i], baseline_phs[i], mask_phs[i], distribution_info_vars[i])
                    surr_objs.append(surr_obj)

                """ Mean over meta tasks """
                meta_objective = tf.reduce_mean(input_tensor=tf.stack(surr_objs, 0))
    ...
    def _make_dice_input_placeholders(self, prefix=''):
        ...
        """
        obs_phs, action_phs, reward_phs, baseline_phs, mask_phs, dist_info_phs = [], [], [], [], [], []
        ...
        for task_id in range(self.meta_batch_size):
            # observation ph
            ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.max_path_length, self.policy.obs_dim],
                                name='obs' + '_' + prefix + '_' + str(task_id))
            all_phs_dict['%s_task%i_%s' % (prefix, task_id, 'observations')] = ph
            obs_phs.append(ph)

            # action ph
            ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.max_path_length, self.policy.action_dim],
                                name='action' + '_' + prefix + '_' + str(task_id))
            all_phs_dict['%s_task%i_%s' % (prefix, task_id, 'actions')] = ph
            action_phs.append(ph)

            # reward ph
            ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.max_path_length], name='rewards' + '_' + prefix + '_' + str(task_id))
            all_phs_dict['%s_task%i_%s' % (prefix, task_id, 'rewards')] = ph
            reward_phs.append(ph)

            # baseline ph
            ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.max_path_length], name='baselines' + '_' + prefix + '_' + str(task_id))
            all_phs_dict['%s_task%i_%s' % (prefix, task_id, 'baselines')] = ph
            baseline_phs.append(ph)

            # mask ph
            ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.max_path_length],
            ...
            for info_key, shape in dist_info_specs:
                ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.max_path_length] + list(shape),
        ...
        return obs_phs, action_phs, reward_phs, baseline_phs, mask_phs, dist_info_phs, all_phs_dict
...
def magic_box_cumsum(logprobs):
    ...
    """
    tf.compat.v1.assert_rank(logprobs, 2)
    with tf.compat.v1.variable_scope("magic_box_cumsum"):
...
def magic_box(logprobs):
    ...
    """
    tf.compat.v1.assert_rank(logprobs, 2)
    with tf.compat.v1.variable_scope("magic_box"):
        tau = logprobs
        magic_box = tf.exp(tau - tf.stop_gradient(tau))
    return magic_box