...
class VPGSGMRL(MAMLAlgo):
    """
    Algorithm for VPG SG-MRL
    ...
    def __init__(
        ...
        super(VPGSGMRL, self).__init__(*args, **kwargs)
        assert inner_type in ["log_likelihood", "likelihood_ratio"]

        self.optimizer = MAMLFirstOrderOptimizer(learning_rate=learning_rate)  # outer
        self.inner_type = inner_type
        self._optimization_keys = ['observations',
                                   'actions',
                                   'advantages',
                                   'agent_infos']
        self.name = name
    ...
    def _adapt_objective_sym(self,
        ...
        if self.inner_type == 'likelihood_ratio':
            ...
            with tf.compat.v1.variable_scope("surrogate_loss"):
                surr_obj_adapt = -tf.reduce_mean(input_tensor=likelihood_ratio_adapt * adv_sym)

        elif self.inner_type == 'log_likelihood':
            with tf.compat.v1.variable_scope("log_likelihood"):
                log_likelihood_adapt = self.policy.distribution.log_likelihood_sym(action_sym,
                                                                                   dist_info_new_sym)
            with tf.compat.v1.variable_scope("surrogate_loss"):
                surr_obj_adapt = -tf.reduce_mean(input_tensor=log_likelihood_adapt * adv_sym)
    ...
    def build_graph(self):
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            for step_id in range(1, self.num_inner_grad_steps+1):
                ...
                for i in range(self.meta_batch_size):
                    surr_loss = self._adapt_objective_sym(action_phs[i],
                                                          adv_phs[i],
                                                          dist_info_old_phs[i],
                ...
                step = f'step{step_id}'
                obs_phs, action_phs, adv_phs, dist_info_old_phs, all_phs_dict = self._make_input_placeholders(step)
                self.meta_op_phs_dict.update(all_phs_dict)
            ...
            for i in range(self.meta_batch_size):
                ...
                surr_obj = - tf.reduce_mean(input_tensor=log_likelihood * adv_phs[i])

                if self.exploration:
                    # add adj_avg_reward placeholder
                    adj_avg_rewards = tf.compat.v1.placeholder(dtype=tf.float32,
                                                               shape=[None],
                                                               name=f'adj_avg_rewards_{self.num_inner_grad_steps}_{i}')
                    self.meta_op_phs_dict[f'step{self.num_inner_grad_steps}_task{i}_adj_avg_rewards'] = adj_avg_rewards

                    iap = initial_action_phs[i]
                    idiv = initial_distribution_info_vars[i]
                    log_likelihood_inital = self.policy.distribution.log_likelihood_sym(iap,
                                                                                        idiv)
                    surr_obj += - tf.reduce_mean(input_tensor=adj_avg_rewards) * tf.reduce_mean(input_tensor=log_likelihood_inital)
                # SG-MRL specific
                iap = initial_action_phs[i]
                idiv = initial_distribution_info_vars[i]
                log_likelihood_inital = self.policy.distribution.log_likelihood_sym(iap, idiv)

                sgmrl_term = tf.stop_gradient(tf.reduce_mean(input_tensor=adv_phs[i])) * tf.reduce_sum(input_tensor=log_likelihood_inital)
                surr_obj -= sgmrl_term
