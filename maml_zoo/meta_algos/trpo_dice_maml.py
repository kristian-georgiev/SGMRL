    ...
    def build_graph(self):
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            for i in range(self.meta_batch_size):
                ...
                likelihood_ratio = tf.reshape(likelihood_ratio, shape=tf.shape(input=mask_phs[i]))
                surr_obj = - tf.reduce_mean(input_tensor=mask_phs[i] * likelihood_ratio * adv_phs[i])

                # KL-divergence for constaraint
                kl_sym = self.policy.distribution.kl_sym(dist_info_old_stacked, distribution_info_vars[i])
                kl_sym = tf.reshape(kl_sym, shape=tf.shape(input=mask_phs[i]))
                outer_kl = tf.reduce_mean(input_tensor=mask_phs[i] * kl_sym)
