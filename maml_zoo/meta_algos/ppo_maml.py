        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            for step_id in range(1, self.num_inner_grad_steps+1):
                ...
                for i in range(self.meta_batch_size):
                    ...
                    kl_loss = tf.reduce_mean(input_tensor=self.policy.distribution.kl_sym(dist_info_old_phs[i], distribution_info_vars[i]))
            ...
            mean_inner_kl_per_step = tf.stack([tf.reduce_mean(input_tensor=tf.stack(inner_kls)) for inner_kls in all_inner_kls])

            """ Outer objective """
            surr_objs, outer_kls = [], []

            # Create placeholders
            inner_kl_coeff = tf.compat.v1.placeholder(tf.float32, shape=[self.num_inner_grad_steps], name='inner_kl_coeff')
            self.meta_op_phs_dict['inner_kl_coeff'] = inner_kl_coeff

            if self.clip_outer:
                clip_eps_ph = tf.compat.v1.placeholder(tf.float32, shape=[], name='clip_eps')
                self.meta_op_phs_dict['clip_eps'] = clip_eps_ph
            else:
                outer_kl_coeff = tf.compat.v1.placeholder(tf.float32, shape=[], name='outer_kl_coef')
            ...
            for i in range(self.meta_batch_size):
                ...
                outer_kl = tf.reduce_mean(input_tensor=self.policy.distribution.kl_sym(dist_info_old_phs[i], distribution_info_vars[i]))
                ...
                if self.clip_outer:  # clipped likelihood ratio
                    ...
                    surr_obj = - tf.reduce_mean(input_tensor=clipped_obj)

                else: # outer kl penalty
                    outer_kl_penalty = outer_kl_coeff * outer_kl
                    surr_obj = - tf.reduce_mean(input_tensor=likelihood_ratio * adv_phs[i]) + outer_kl_penalty

                surr_objs.append(surr_obj)
                outer_kls.append(outer_kl)

            mean_outer_kl = tf.reduce_mean(input_tensor=tf.stack(outer_kls))
            inner_kl_penalty = tf.reduce_mean(input_tensor=inner_kl_coeff * mean_inner_kl_per_step)

            """ Mean over meta tasks """
            meta_objective = tf.reduce_mean(input_tensor=tf.stack(surr_objs, 0)) + inner_kl_penalty
