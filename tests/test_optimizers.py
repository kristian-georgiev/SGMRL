...
def fc(x, scope, nh, *, init_scale=1.0, init_bias=0.0):
    with tf.compat.v1.variable_scope(scope):
        ...
        w = tf.compat.v1.get_variable("w", [nin, nh], initializer=tf.compat.v1.orthogonal_initializer(init_scale))
    ...
    def testSine(self):
        for optimizer in [MAMLFirstOrderOptimizer()]:
            ...
            with tf.compat.v1.Session():
                ...
                loss = tf.reduce_mean(input_tensor=tf.square(network.output - target_phs))
    ...
    def testGauss(self):
        for optimizer in [MAMLFirstOrderOptimizer()]:
            ...
            with tf.compat.v1.Session():
                ...
                loss = tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=numerator / denominator + std_network.output - target_std_ph, axis=-1))
