...
class VPG_DICEMAML(DICEMAML):
        ...
        with meta_update_scope:
            ...
            with tf.compat.v1.variable_scope("outer_update"):
                ...
                for i in range(self.meta_batch_size):
                    ...
                    log_likelihood = tf.reshape(log_likelihood, tf.shape(input=mask_phs[i]))
                    surr_obj = - tf.reduce_mean(input_tensor=log_likelihood * adv_phs[i] * mask_phs[i])
    ...
    def _make_advantage_phs(self, prefix=''):
        ...
        for task_id in range(self.meta_batch_size):
            # advantage ph
            ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.max_path_length], name='advantage' + '_' + prefix + '_' + str(task_id))
