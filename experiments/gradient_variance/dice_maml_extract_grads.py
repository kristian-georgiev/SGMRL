...
class DICEMAML(MAMLAlgo):
    ...
    def _adapt_objective_sym(self, action_stacked_sym, adj_reward_sym, mask_sym, dist_info_stacked_sym): #TODO make this method private
        ...
        with tf.compat.v1.variable_scope("dice_loss"):
            obj_adapt = - tf.reduce_mean(input_tensor=magic_box(log_likelihood_adapt) * adj_reward_sym * mask_sym)
        return obj_adapt
    ...
    def build_graph(self):
        ...
        with meta_update_scope:
            ...
            with tf.compat.v1.variable_scope("outer_update"):
                ...
                meta_gradients = tf.gradients(ys=meta_objective, xs=[params_var[key] for key in sorted(params_var.keys())])
    ...
    def compute_gradients(self, all_samples_data, log=True):
        ...
        gradients_values = tf.compat.v1.get_default_session().run(self.gradients, feed_dict=feed_dict)
    ...
    def _make_dice_input_placeholders(self, prefix=''):
        ...
        for task_id in range(self.meta_batch_size):
            ...
            # adjusted reward ph
            ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.max_path_length], name='adjusted_rewards' + '_' + prefix + '_' + str(task_id))
    ...
    def _adapt_sym(self, surr_obj, params_var):
        ...
        grads = tf.gradients(ys=surr_obj, xs=[params_var[key] for key in update_param_keys])
