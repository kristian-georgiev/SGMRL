    ...
    def build_graph(self):
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            for i in range(self.meta_batch_size):
                ...
                surr_obj = - tf.reduce_mean(input_tensor=likelihood_ratio * adv_phs[i])

                if self.exploration:
                    # add adj_avg_reward placeholder
                    adj_avg_rewards = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None], name='adj_avg_rewards' + '_' + str(
                    ...
                    log_likelihood_inital = self.policy.distribution.log_likelihood_sym(initial_action_phs[i],
                                                                                        initial_distribution_info_vars[
                                                                                            i])
                    surr_obj += - tf.reduce_mean(input_tensor=adj_avg_rewards) * tf.reduce_mean(input_tensor=log_likelihood_inital) * self.rollouts_per_meta_task
